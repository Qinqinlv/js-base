...
window.onload  = function () {
    ...
    //初始情况下，除了第一张图片留在盒子的左边，其它的图片都存放在盒子的右边
    var scrollWidth = js_silder.clientWidth;
    ...
    for(var k in spans){
        spans[k].onclick = function () {
            if(this.className == "slider-ctrl-prev"){
                //点击左边的按钮，图片left:-310 -- 0;
                // 紧接着的第二张图片的left值迅速跳转到-310的位置，准备下一次的left从-310到0的动画
                animate(imgs[key],{left:310});
                ...
                //key++;
                ////图片经过第一轮移动后，都从右边移动到了left为-310的位置叠起来了
                //// 第二轮图片轮播的时候，key的值要变成0，从头开始
                //if(key > imgs.length - 1){
                //    key = 0;
                //    //当前图片滑动方向（left:0 -- -310）
                //    //从右向左滑动到舞台中央（left：310 -- 0）
                //    //新一轮图片轮播，每次图片从左边往右边滑过来，和第一轮方向是反的，而且以后的每一次都是从左往右
                //    //为了让图片能够每一轮播放的时候都是保持left从310 -- 0的方向，
                //    // 在当前图片从右到左滑动后，紧接着第二张图片要快速跳回到left = 310的位置，准备从310 -- 0
                //}
                //imgs[key].style["left"] = scrollWidth + "px";
                //animate(imgs[key],{left:0});
                autoPlay();
                ...
                //当被点击的span索引大于key的时候，当前的图片（也就是索引为key）的这张，向左边移动
                // 位于key右边的that索引的图片从left=310到0移动。
                if(that > key){
                    animate(imgs[key],{left:-scrollWidth});
                    imgs[that].style.left = scrollWidth + "px";
                }
                //当被点击的span索引小于key的时候，当前的图片（也就是索引为key）的这张，向右边移动
                // 位于key右边的that索引的图片从left=-310到0移动。
                else if(that < key){
                    animate(imgs[key],{left:scrollWidth});
                    imgs[that].style.left = -scrollWidth + "px";
                }
                //将that的值赋给key,这样下次使用左右两边的大于、小于号来滚动图片的时候，会首先将key = that得到的
                //key值再key++或者是key--，接着播放key后的动画。如果不把当前的that值赋给key，比如that的值是2，
                //key的值本来是4，动画之后索引为5的图片移动到了舞台中央，但是key还是5，当点击右边的大于号的时候，播放索引
                //为key++的第6张图片。效果是：前一张图片是第三张图片，接下来播放的是第6张图片，就混乱了
                key = that;
                animate(imgs[that],{left:0});
                setSquare();
                //这句话本来是写在if和else if里面的，但是最后一种情况是key = that,当key=that的时候，图片不动，
                // 就算写上animate(imgs[that],{left:0}),由于它的left本来就已经是0了，所以也不会动。
    ...
    function autoPlay(){
        //其实这个自动播放的定时器的作用和右边按钮的功能是一样的——图片不停的从右往左移动。
